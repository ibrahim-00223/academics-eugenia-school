...
export default async function DashboardPage() {
  ...
  const [
    ...
    { data: topSkills },
    { data: recentJobs },
    { data: scrapingSources },
    { data: latestReport },
  ] = await Promise.all([
    supabase.from('raw_job_posts').select('*', { count: 'exact', head: true }),
    ...
    supabase
      .from('raw_job_posts')
      .select('id, title, company, scraped_at, source_id')
      .order('scraped_at', { ascending: false })
      .limit(5),
    supabase
      .from('scraping_sources')
      .select('id, name, is_active, last_successful_scrape_at')
      .order('name'),
    supabase
  ...
  const staleSources = (scrapingSources ?? []).filter((s) => {
  ...
  const statsCards = [
    {
      ...
      value: totalJobPosts?.toLocaleString('fr-FR') ?? '—',
    ...
    },
    {
      ...
      value: totalSkills?.toLocaleString('fr-FR') ?? '—',
    ...
    },
    {
      ...
      value: totalPrograms?.toLocaleString('fr-FR') ?? '—',
    ...
    },
    {
      ...
      value: latestReport
        ...
        : 'Aucun',
      description: latestReport?.title ?? 'Pas encore généré',
      icon: FileText,
            ...
            {topSkills && topSkills.length > 0 ? (
              <div className="space-y-3">
                {topSkills.map((item, i) => {
                  const skillName = (item.skill_taxonomy as unknown as { name: string } | null)?.name ?? 'Inconnu'
